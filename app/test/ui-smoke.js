...
;(async () => {
  ...
  assert.equal(await js('gridColumns()'), 2)
  assert.ok(await js('listScroll.clientHeight > 200 && [...rows.values()].every(({item})=>item.getBoundingClientRect().bottom <= listScroll.getBoundingClientRect().bottom + 1)'), 'home tools are visible inside the compact grid')
  assert.ok(await js('parseFloat(getComputedStyle(document.querySelector(".cli-item")).fontSize) >= 22'))
  const outputDir = path.resolve(__dirname, '../../out/review')
  await fs.mkdir(outputDir, { recursive: true })
  await fs.writeFile(path.join(outputDir, 'compact-home.png'), (await win.webContents.capturePage()).toPNG())
  ...
  assert.equal(extra.isDestroyed(),true,'auxiliary window closes when its CLI exits')
  await js('window.api.ptyWrite("done" + String.fromCharCode(13))')
  for(let attempt=0;attempt<100 && await js('currentView')!== 'list';attempt++) await delay(30)
  assert.equal(await js('currentView'),'list','primary CLI exit returns to the home screen')
  win.setSize(480,340)
  await delay(100)
